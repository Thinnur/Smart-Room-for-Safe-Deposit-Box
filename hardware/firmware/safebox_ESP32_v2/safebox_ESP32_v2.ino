 ...
 *  └─ finger.begin() dicoba di dua baud rate (57600 lalu fallback 9600)
 *      untuk support klon AS608 dengan baud default berbeda
 *
 ...
 *
 *  WIRING RINGKASAN:
 *  Tombol OTP   → BTN1..BTN6: GPIO 18, 19, 21, 22, 23, 32 (INPUT_PULLUP, aktif LOW)
...
// Fingerprint AS608 via Serial2 (default GPIO 16 RX, 17 TX)
...
#define LED_HIJAU  26   // Standby / akses OK
#define LED_KUNING 27   // Memproses
#define LED_MERAH  14   // Ditolak / alarm
...
// ============================================================
#define DURASI_PINTU_BUKA  4000UL     // ms — berapa lama pintu tetap buka
...
//  OBJEK HARDWARE
// ============================================================
Adafruit_Fingerprint finger(&Serial2);   // Serial2 = GPIO 16 RX, 17 TX
...
void setup() {
  ...
  Serial.println("[HW] Tombol OTP OK (GPIO 18,19,21,22,23,32)");

  // ── Fingerprint (Hardware Serial2: RX=16, TX=17) ─────────
  // [v2] Coba 57600 dulu, fallback ke 9600 untuk klon AS608
  // Serial2 default pins ESP32 = GPIO16 (RX), GPIO17 (TX) ✓
  finger.begin(57600);
  delay(200);
  if (finger.verifyPassword()) {
    Serial.println("[FP] Sensor OK (57600) — kapasitas: " + String(finger.capacity));
  } else {
    Serial.println("[FP] 57600 gagal, coba 9600...");
    finger.begin(9600);
    delay(200);
    if (finger.verifyPassword()) {
      Serial.println("[FP] Sensor OK (9600) — kapasitas: " + String(finger.capacity));
    } else {
      Serial.println("[FP] ERROR! Cek wiring AS608 (TX→GPIO16, RX←GPIO17).");
      setLED(LED_REJECTED);
      buzzerError();
      delay(2000);
...
void loop() {
  ...
  switch (state) {
    ...
    case ALARM:        /* handled inside triggerAlarm() */ break;
  }
}
...
// ============================================================
//  ALARM — LED Merah blink + buzzer 5 detik (blocking)
// ============================================================
void triggerAlarm() {
  State prevState = state;
  state = ALARM;
  Serial.println("[ALARM] !!! Pelanggaran keamanan !!!");
  ...
  digitalWrite(BUZZER, LOW);
  setLED(LED_STANDBY_OK);
  Serial.println("[ALARM] Selesai. Kembali ke " + String(stateStr()));
}
